...
- `veqpy/operator/__init__.py` 导出 `build_profile_layout`, `decode_packed_blocks`, `Operator`, `OperatorCase`, `PROFILE_INDEX`, `PROFILE_NAMES`.

## Current Fact

- packed state 和 packed residual 的唯一位置语义都是 `coeff_index` / `coeff_indices`.
- 当前 runtime 路径不再维护 `coeff_matrix`.
- `Stage-A` 直接从 packed `x` 读取 profile 系数.
...
- `Stage-A` 当前保留 per-profile Python loop.
- `Stage-D` 当前保留 residual block registry + per-block Python dispatch.
...
# Hot-Path ABI Guardrail

## Contract

public/model 语义可以 richer, 但 hot engine ABI 默认必须保持单态.
...
- engine 热路径优先只接收:
  ...
  - `int`
  - 显式索引数组 / 长度数组 / code 数组
- 进入 engine 前优先 lower 成 field bundle 或 packed index 语义.
- 优先使用:
  - `T_fields`
  - `u_fields`
  - `rp_fields`
...
- 不把 `None` 直接塞进 hot Numba kernels, 除非有 benchmark 证明可接受.
- 不在 hot path 中重新引入 exploded long-argument ABI, 前提是稳定 field bundle 已经存在.
- 不在 packed 热路径中重新引入 `coeff_matrix`.

## Rationale

当前分支已经验证:

- `coeff_indices` 比中间 row-cache 更适合作为 packed ABI;
- field-bundle ABI 可以明显降低 Stage-B / Stage-D 的 Python 外壳成本;
- direct `None` in hot Numba kernels 会导致明显性能回退.
